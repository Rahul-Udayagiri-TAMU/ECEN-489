...
int main() {
    ...
    // Copy result back
    cudaMemcpy(h_output, d_output, NUM_FILTERS * OUTPUT_SIZE * OUTPUT_SIZE * sizeof(float), cudaMemcpyDeviceToHost);

    // Save output
    save_output("conv1_cuda_output.txt", h_output);
...
// Print predicted class
    ...
    // Free memory
//    free(h_input); free(h_filters); free(h_biases); free(h_output);
